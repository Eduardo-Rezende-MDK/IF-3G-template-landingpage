...
| **`Reasons`** _(Opcional)_ | `Reasons.tsx`      | Componente modular alternativo com 6 diferenciais da 3G Foods (portfólio, abastecimento, distribuição, atendimento, compra online, qualidade).                                                                                                    |
...
2. **SEO Perfeito:** Mecanismos de busca (Google, Bing) e agentes de IA recebem imediatamente o conteúdo completo sem aguardar a fila de execução JavaScript (_Two-Wave Indexing_).
...
Para lançar uma nova landing page (ex: _Queijo Prato_, _Batata Congelada_, _Catupiry_, _Carnes para Hamburgueria_) usando este template:
...
## 10. Blueprint evolutivo e extensibilidade

O blueprint deixa de ser apenas um mapa fixo de dez blocos e passa a ter três camadas: **núcleo de componentes**, **manifesto de secções** e **configuração de campanha**.

O núcleo vive em `src/components/landing/` e contém componentes reutilizáveis. O manifesto vive em `src/data/landing.config.json` na chave `sections` e determina a ordem, o tipo, o identificador e a activação de cada secção. A configuração de conteúdo, produto, SEO, tracking e schema permanece no mesmo ficheiro, enquanto `src/data/landing.ts` fornece um adaptador tipado para os componentes.
...
```json
{
  "sections": [
    ...
    { "id": "applications", "type": "applications", "enabled": false },
    { "id": "faq", "type": "faq", "enabled": true }
  ]
}
```

`SectionRenderer.tsx` funciona como registry. Para criar uma nova secção reutilizável, Rita deve criar o componente, adicionar o tipo ao registry, definir os dados necessários no schema de configuração, incluir validação, documentar eventos e acrescentar pelo menos uma fixture ou estado de QA. Para personalizar uma campanha, não se deve duplicar o `App.tsx`; deve-se alterar a configuração, activar/ocultar/reordenar secções ou criar uma extensão no repositório próprio da campanha.

### Regras de evolução

| Necessidade                                                   | Local correcto                                                           |
| ------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Corrigir bug comum de botão, card, tracking ou acessibilidade | Template-base, com branch e pull request                                 |
| Criar novo tipo de secção reutilizável                        | Template-base, com componente, registry, tipos, validação e documentação |
| Alterar ordem ou activar/desactivar uma secção numa campanha  | Repositório próprio da campanha, no manifesto/configuração               |
| Trocar copy, produto, preço, imagem, URL, FAQ ou claim        | Repositório próprio da campanha                                          |
| Criar uma excepção visual exclusiva                           | Repositório próprio, sem contaminar o componente-base                    |
| Promover uma excepção para capacidade comum                   | Refactorizar no template-base e versionar uma nova release               |

A regra de governança é: **melhorias comuns entram no template; decisões específicas entram no repositório novo da landing**. Cada landing deve registar a versão de origem do template e não deve receber alterações automáticas sem QA.

### Contratos para novas secções

Cada nova secção deve ter um `id` estável, um `type` conhecido, conteúdo configurável, estado vazio, estado de erro, comportamento mobile, heading semântico, acessibilidade, tracking quando necessário e critérios de aceitação. O `validate-campaign.mjs` deve rejeitar tipos desconhecidos, IDs duplicados e campos essenciais vazios.

Esse modelo facilita a evolução sem transformar o template num monólito: novos blocos podem ser acrescentados uma vez, campanhas podem escolher os blocos que precisam e o `App.tsx` permanece estável.
